# QoreChain Hub

QoreChain ekosistemi icin Turkce ve Ingilizce teknik kaynaklar, node operasyon notlari, gorev surecleri ve topluluk odakli bilgi merkezi.

A bilingual knowledge hub for QoreChain technical resources, node operations, task workflows, and community-focused documentation.

## Dil Secimi / Language

| Dil | Bolum | Durum |
|---|---|---|
| Turkce | [Turkce Kaynaklar](#turkce-kaynaklar) | Aktif |
| English | [English Resources](#english-resources) | Preparing |

## Turkce Kaynaklar

Bu bolum, QoreChain ile ilgilenen Turkce toplulugun kurulum, kontrol, takip ve sorun cozme sureclerini daha duzenli bir sekilde yurutmesine yardimci olmak icin hazirlanmistir.

### Odak Alanlari

- Light Node kurulumu ve operasyon takibi
- Eigenstate 2 gorev surecleri
- QoreChain ekosistem notlari
- Temel sorun giderme adimlari
- Operatorler icin pratik kontrol listeleri
- Turkce teknik dokumantasyon

### Icerik Haritasi

| Kaynak | Icerik | Durum |
|---|---|---|
| [Light Node Operasyonlari](./LIGHTNODE_SETUP.md) | Kurulum, servis kontrolu, panel erisimi ve temel sorun giderme | Aktif |
| [QoreChain Bilgi Bankasi](./FAQ.md) | Light Node, stake ve temel ekosistem sorulari | Aktif |
| Eigenstate 2 Gorevleri | Gorev akisi, kontrol adimlari ve notlar | Hazirlaniyor |
| Operator El Kitabi | Log kontrolu, yeniden baslatma, guncelleme ve takip surecleri | Planlandi |
| Hata Cozumleri | Sik karsilasilan hatalar ve pratik cozumler | Planlandi |
| Guncelleme Notlari | Yeni gorevler, degisiklikler ve takip notlari | Planlandi |

## English Resources

This section will include English documentation for QoreChain users, operators, and community members who want to follow setup flows, node operations, task processes, and troubleshooting notes.

### Focus Areas

- Light Node setup and operations
- Eigenstate 2 task workflows
- QoreChain ecosystem notes
- Basic troubleshooting steps
- Operator checklists
- English technical documentation

### Content Map

| Resource | Content | Status |
|---|---|---|
| Light Node Operations | Setup, service checks, panel access, and basic troubleshooting | Preparing |
| QoreChain Knowledge Base | Light Node, stake, and ecosystem questions | Preparing |
| Eigenstate 2 Tasks | Task flow, verification steps, and notes | Preparing |
| Operator Handbook | Logs, restart, updates, and monitoring process | Planned |
| Troubleshooting | Common issues and practical fixes | Planned |
| Update Notes | New tasks, changes, and tracking notes | Planned |

## QoreChain Hakkinda / About QoreChain

QoreChain, kuantum sonrasi guvenlik yaklasimini merkeze alan yeni nesil bir Katman 1 blokzincir altyapisidir. EVM uyumlulugu, CosmWasm destegi, SVM destegi, yapay zeka odakli altyapi ve aglar arasi dogrulayici mimarisi gibi basliklarla merkeziyetsiz altyapinin gelecek ihtiyaclarina odaklanir.

QoreChain is a next-generation Layer 1 blockchain infrastructure focused on post-quantum security, EVM compatibility, CosmWasm support, SVM support, AI-native infrastructure, and cross-network validator architecture.

## Calisma Yaklasimi / Documentation Approach

QoreChain Hub icerikleri mumkun oldugunca sade, duzenli ve uygulanabilir tutulur. Amac, topluluktaki kullanicilarin teknik surecleri tek tek aramak yerine tek bir merkezden takip edebilmesidir.

QoreChain Hub content is designed to stay clear, structured, and practical. The goal is to help users follow technical processes from a single organized source.

Her kaynakta su yapi korunur:

- Net amac
- Gereksinimler
- Adim adim uygulama
- Kontrol komutlari
- Sik karsilasilan sorunlar
- Guncelleme notlari

Each resource follows a consistent structure:

- Clear objective
- Requirements
- Step-by-step flow
- Verification commands
- Common issues
- Update notes

## Baglantili Projeler / Related Projects

- [QoreChain Light Node](https://github.com/satoshi-Qore/qorechain-lightnode)
- [QoreChain Araclari / Tools](https://github.com/satoshi-Qore/qorechain-tools)
- [QoreChain Notlari / Notes](https://github.com/satoshi-Qore/Qorechain-notes)
- [Satoshi-Qore GitHub Profili / Profile](https://github.com/satoshi-Qore)

## Katki ve Geri Bildirim / Contribution and Feedback

Bu alan topluluk icin hazirlanmistir. Eksik gordugunuz konular, yeni gorev notlari, kurulum iyilestirmeleri veya sorun cozumu onerileri icin issue acabilir ya da pull request gonderebilirsiniz.

This hub is prepared for the community. You can open an issue or submit a pull request for missing topics, new task notes, setup improvements, or troubleshooting suggestions.

## Not / Disclaimer

Bu depo topluluk tarafindan hazirlanan bagimsiz bir bilgi merkezidir. Resmi QoreChain dokumantasyonu yerine gecmez; kritik adimlarda resmi kaynaklar ve guncel duyurular kontrol edilmelidir.

This is an independent community-maintained knowledge hub. It does not replace official QoreChain documentation; official sources and current announcements should be checked for critical steps.
